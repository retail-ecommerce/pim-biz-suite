...
import styles from './BrandFilter.preference.less'
...
const internalImageListOf = (brandFilter) =>{
...
const internalSettingListOf = (brandFilter) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = BrandFilterService[candidateServiceName] 
...
const executeTrans = (brandFilter,targetComponent) =>{
...
const buildTransferModal = (brandFilter,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(brandFilter,targetComponent)}
...
const internalRenderExtraHeader = (brandFilter) =>{
	return null
}
const internalRenderExtraFooter = (brandFilter) =>{
	return null
}
...
<Description term="Profile">{brandFilter.profile==null?appLocaleName(userContext,"NotAssigned"):brandFilter.profile.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"Profile","profile","requestCandidateProfile",
	      "transferToAnotherProfile","anotherProfileId",brandFilter.profile?brandFilter.profile.id:"")} 
...
const renderPermissionSetting = brandFilter => {
  const {BrandFilterBase} = GlobalComponents
  return <PermissionSetting targetObject={brandFilter}  targetObjectMeta={BrandFilterBase}/>


}


class BrandFilterPreference extends Component {
  ...
  render() {
    ...
    const cardsData = {cardsName:"Brand Filter",cardsFor: "brandFilter",cardsSource: this.props.brandFilter,
  		subItems: [
...
}))(Form.create()(BrandFilterPreference))
