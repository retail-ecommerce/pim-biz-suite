...
import styles from './BrandFilter.table.less'
...
class BrandFilterTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
 ...
 calcDisplayColumns=()=>{
    ...
    const {displayColumns} = BrandFilterBase
    if(!referenceName){
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
         ...
         { hasReadPermission(metaInfo)&&<Link to={`/brandFilter/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
