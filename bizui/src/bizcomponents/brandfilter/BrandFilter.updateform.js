...
import styles from './BrandFilter.updateform.less'
...
class BrandFilterUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const brandFilterId = values.id
        const imagesValues = mapBackToImageValues(convertedImagesValues)
        const parameters = { ...values, brandFilterId, ...imagesValues }
    ...
    const submitUpdateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/updateBrandFilter`,
          payload: {
            id: owner.id,
            type: 'brandFilter',
            parameters,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextBrandFilterUpdateRow`,
        payload: {
          id: owner.id,
          type: 'brandFilter',
          selectedRows,
              ...
              <Col lg={12} md={12} sm={24}>
                <Form.Item label={fieldLabels.internalId} {...formItemLayout}>
                  {getFieldDecorator('internalId', {
                    initialValue: selectedRow.internalId,
                    rules: [{ required: true, message: appLocaleName(userContext,"PleaseInput") }],
                  })(
...
}))(Form.create()(BrandFilterUpdateForm))
