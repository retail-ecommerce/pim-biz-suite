...
import styles from './EditorPickProduct.table.less'
...
class EditorPickProductTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
 ...
 calcDisplayColumns=()=>{
    ...
    const {displayColumns} = EditorPickProductBase
    if(!referenceName){
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
         ...
         { hasReadPermission(metaInfo)&&<Link to={`/editorPickProduct/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
...
export default EditorPickProductTable
