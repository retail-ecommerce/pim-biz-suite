...
const addProduct = (targetObjectId, parameters) => {
  const url = `${PREFIX}levelNCategoryManager/addProduct/levelNCategoryId/displayName/brandId/origin/catalogId/remark/tokensExpr/`
...
const updateProduct = (targetObjectId, parameters) => {
  const url = `${PREFIX}levelNCategoryManager/updateProductProperties/levelNCategoryId/id/displayName/origin/remark/tokensExpr/`
  const levelNCategoryId = targetObjectId
  const requestParameters = { ...parameters, levelNCategoryId, tokensExpr: 'none' }
  return postForm({ url,requestParameters})
}
