...
import styles from './RecommandProduct.preference.less'
...
const internalImageListOf = (recommandProduct) =>{
...
const internalSettingListOf = (recommandProduct) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = RecommandProductService[candidateServiceName] 
...
const executeTrans = (recommandProduct,targetComponent) =>{
...
const buildTransferModal = (recommandProduct,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(recommandProduct,targetComponent)}
...
const internalRenderExtraHeader = (recommandProduct) =>{
	return null
}
const internalRenderExtraFooter = (recommandProduct) =>{
	return null
}
...
<Description term="Parent Category">{recommandProduct.parentCategory==null?appLocaleName(userContext,"NotAssigned"):recommandProduct.parentCategory.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"Parent Category","levelNCategory","requestCandidateParentCategory",
	      "transferToAnotherParentCategory","anotherParentCategoryId",recommandProduct.parentCategory?recommandProduct.parentCategory.id:"")} 
...
<Description term="Brand">{recommandProduct.brand==null?appLocaleName(userContext,"NotAssigned"):recommandProduct.brand.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"Brand","brand","requestCandidateBrand",
	      "transferToAnotherBrand","anotherBrandId",recommandProduct.brand?recommandProduct.brand.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
...
<Description term="Catalog">{recommandProduct.catalog==null?appLocaleName(userContext,"NotAssigned"):recommandProduct.catalog.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"Catalog","catalog","requestCandidateCatalog",
	      "transferToAnotherCatalog","anotherCatalogId",recommandProduct.catalog?recommandProduct.catalog.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
...
<Description term="Profile">{recommandProduct.profile==null?appLocaleName(userContext,"NotAssigned"):recommandProduct.profile.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"Profile","profile","requestCandidateProfile",
	      "transferToAnotherProfile","anotherProfileId",recommandProduct.profile?recommandProduct.profile.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
...
const renderPermissionSetting = recommandProduct => {
  const {RecommandProductBase} = GlobalComponents
  return <PermissionSetting targetObject={recommandProduct}  targetObjectMeta={RecommandProductBase}/>


}


class RecommandProductPreference extends Component {
  ...
  render() {
    ...
    const cardsData = {cardsName:"Recommand Product",cardsFor: "recommandProduct",cardsSource: this.props.recommandProduct,
  		subItems: [
...
}))(Form.create()(RecommandProductPreference))
