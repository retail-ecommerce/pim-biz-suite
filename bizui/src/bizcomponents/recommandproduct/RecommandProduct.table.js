...
import styles from './RecommandProduct.table.less'
...
class RecommandProductTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
 ...
 calcDisplayColumns=()=>{
    ...
    const {displayColumns} = RecommandProductBase
    if(!referenceName){
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
         ...
         { hasReadPermission(metaInfo)&&<Link to={`/recommandProduct/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
...
export default RecommandProductTable
