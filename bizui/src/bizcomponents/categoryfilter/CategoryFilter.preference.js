...
import styles from './CategoryFilter.preference.less'
...
const internalImageListOf = (categoryFilter) =>{
...
const internalSettingListOf = (categoryFilter) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = CategoryFilterService[candidateServiceName] 
...
const executeTrans = (categoryFilter,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_categoryFilter/doJob",payload: payload})
...
const buildTransferModal = (categoryFilter,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(categoryFilter,targetComponent)}
...
const internalRenderExtraHeader = (categoryFilter) =>{
	return null
}
const internalRenderExtraFooter = (categoryFilter) =>{
	return null
}
...
<Description term="Profile">{categoryFilter.profile==null?appLocaleName(userContext,"NotAssigned"):categoryFilter.profile.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"Profile","profile","requestCandidateProfile",
	      "transferToAnotherProfile","anotherProfileId",categoryFilter.profile?categoryFilter.profile.id:"")} 
...
const renderPermissionSetting = categoryFilter => {
  const {CategoryFilterBase} = GlobalComponents
  return <PermissionSetting targetObject={categoryFilter}  targetObjectMeta={CategoryFilterBase}/>


}


class CategoryFilterPreference extends Component {
  ...
  render() {
    ...
    const cardsData = {cardsName:"Category Filter",cardsFor: "categoryFilter",cardsSource: this.props.categoryFilter,
  		subItems: [
...
}))(Form.create()(CategoryFilterPreference))
