...
class CategoryFilterTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
         ...
         { hasReadPermission(metaInfo)&&<Link to={`/categoryFilter/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
