...
import styles from './NewProduct.preference.less'
...
const internalImageListOf = (newProduct) =>{
...
const internalSettingListOf = (newProduct) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = NewProductService[candidateServiceName] 
...
const executeTrans = (newProduct,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_newProduct/doJob",payload: payload})
...
const buildTransferModal = (newProduct,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(newProduct,targetComponent)}
...
const internalRenderExtraHeader = (newProduct) =>{
	return null
}
const internalRenderExtraFooter = (newProduct) =>{
	return null
}
...
<Description term="Parent Category">{newProduct.parentCategory==null?appLocaleName(userContext,"NotAssigned"):newProduct.parentCategory.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"Parent Category","levelNCategory","requestCandidateParentCategory",
	      "transferToAnotherParentCategory","anotherParentCategoryId",newProduct.parentCategory?newProduct.parentCategory.id:"")} 
...
<Description term="Brand">{newProduct.brand==null?appLocaleName(userContext,"NotAssigned"):newProduct.brand.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"Brand","brand","requestCandidateBrand",
	      "transferToAnotherBrand","anotherBrandId",newProduct.brand?newProduct.brand.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
...
<Description term="Catalog">{newProduct.catalog==null?appLocaleName(userContext,"NotAssigned"):newProduct.catalog.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"Catalog","catalog","requestCandidateCatalog",
	      "transferToAnotherCatalog","anotherCatalogId",newProduct.catalog?newProduct.catalog.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="Profile">{newProduct.profile==null?appLocaleName(userContext,"NotAssigned"):newProduct.profile.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"Profile","profile","requestCandidateProfile",
	      "transferToAnotherProfile","anotherProfileId",newProduct.profile?newProduct.profile.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
...
const renderPermissionSetting = newProduct => {
  const {NewProductBase} = GlobalComponents
  return <PermissionSetting targetObject={newProduct}  targetObjectMeta={NewProductBase}/>


}


class NewProductPreference extends Component {
  ...
  render() {
    ...
    const cardsData = {cardsName:"New Product",cardsFor: "newProduct",cardsSource: this.props.newProduct,
  		subItems: [
...
}))(Form.create()(NewProductPreference))
