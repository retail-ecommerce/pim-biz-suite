...
import styles from './NewProduct.createform.less'
...
class NewProductCreateForm extends Component {
  state = {
  ...
  render() {
    ...
    const {fieldLabels} = NewProductBase
    ...
    const submitCreateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const parameters = { ...values, ...imagesValues }
        dispatch({
          type: `${owner.type}/addNewProduct`,
          payload: { id: owner.id, type: 'newProduct', parameters, continueNext: true },
        })
      })
    }
    ...
    const goback = () => {
      ...
      dispatch({
        ...
        payload: { id: owner.id, type: 'newProduct',listName:appLocaleName(userContext,"List") },
      })
    }
...
}))(Form.create()(NewProductCreateForm))
