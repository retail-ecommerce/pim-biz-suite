...
import styles from './PriceFilter.preference.less'
...
const internalImageListOf = (priceFilter) =>{
...
const internalSettingListOf = (priceFilter) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = PriceFilterService[candidateServiceName] 
...
const executeTrans = (priceFilter,targetComponent) =>{
...
const buildTransferModal = (priceFilter,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(priceFilter,targetComponent)}
...
const internalRenderExtraHeader = (priceFilter) =>{
	return null
}
const internalRenderExtraFooter = (priceFilter) =>{
	return null
}
...
<Description term="Profile">{priceFilter.profile==null?appLocaleName(userContext,"NotAssigned"):priceFilter.profile.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"Profile","profile","requestCandidateProfile",
	      "transferToAnotherProfile","anotherProfileId",priceFilter.profile?priceFilter.profile.id:"")} 
...
const renderPermissionSetting = priceFilter => {
  const {PriceFilterBase} = GlobalComponents
  return <PermissionSetting targetObject={priceFilter}  targetObjectMeta={PriceFilterBase}/>


}


class PriceFilterPreference extends Component {
  ...
  render() {
    ...
    const cardsData = {cardsName:"Price Filter",cardsFor: "priceFilter",cardsSource: this.props.priceFilter,
  		subItems: [
...
}))(Form.create()(PriceFilterPreference))
