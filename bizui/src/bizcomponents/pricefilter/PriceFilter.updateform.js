...
import styles from './PriceFilter.updateform.less'
...
class PriceFilterUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const priceFilterId = values.id
        const imagesValues = mapBackToImageValues(convertedImagesValues)
        const parameters = { ...values, priceFilterId, ...imagesValues }
    ...
    const submitUpdateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/updatePriceFilter`,
          payload: {
            id: owner.id,
            type: 'priceFilter',
            parameters,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextPriceFilterUpdateRow`,
        payload: {
          id: owner.id,
          type: 'priceFilter',
          selectedRows,
              ...
              <Col lg={12} md={12} sm={24}>
                <Form.Item label={fieldLabels.priceStart} {...formItemLayout}>
                  {getFieldDecorator('priceStart', {
                    initialValue: selectedRow.priceStart,
                    rules: [{ required: true, message: appLocaleName(userContext,"PleaseInput") }],
                  })(
              ...
              <Col lg={12} md={12} sm={24}>
                <Form.Item label={fieldLabels.priceEnd} {...formItemLayout}>
                  {getFieldDecorator('priceEnd', {
                    initialValue: selectedRow.priceEnd,
                    rules: [{ required: true, message: appLocaleName(userContext,"PleaseInput") }],
                  })(
...
}))(Form.create()(PriceFilterUpdateForm))
