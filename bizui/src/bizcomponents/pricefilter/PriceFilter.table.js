...
import styles from './PriceFilter.table.less'
...
class PriceFilterTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
 ...
 calcDisplayColumns=()=>{
    ...
    const {displayColumns} = PriceFilterBase
    if(!referenceName){
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
         ...
         { hasReadPermission(metaInfo)&&<Link to={`/priceFilter/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
